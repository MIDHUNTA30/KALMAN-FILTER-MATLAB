...
Ppd=zeros(n,N); % Ppd stores the diagonal elements of Pk|k-1
Pf=zeros(n,n*(N+1));  % Pf stores the sequence of matrices Pk|k
Pf(:,1:n)=Q;
Pfd=zeros(n,N+1);  % Psd stores the diagonal elements of Pk|k
Pfd(:,1)=diag(Q);
...
time = (0:N);
figure(1)
plot(time(1:end-1),Ppd(1,:),'k.-',time,Pfd(1,:),'r.-',time,Psd(1,:),'g.-','LineWidth',1) 
legend('${P}_{{k|k-1}_{11}}$','${P}_{{k|k}_{11}}$','${P}_{{k|N}_{11}}$','Interpreter','latex');
axis([0 20 0 4])
xlabel('k','Interpreter','latex');ylabel('$\textbf{P}_{k|k-1},\textbf{P}_{k|k},\textbf{P}_{k|N}$','Interpreter','latex');
grid on
ax = gca;
ax.GridAlpha = 1
ax.GridLineStyle = ':'
...
print -dsvg fig2a
figure(2)
plot(time(1:end-1),Ppd(2,:),'k.-',time,Pfd(2,:),'r.-',time,Psd(2,:),'g.-','LineWidth',1) 
legend('${P}_{{k|k-1}_{22}}$','${P}_{{k|k}_{22}}$','${P}_{{k|N}_{22}}$','Interpreter','latex');
axis([0 20 0 40])
xlabel('k','Interpreter','latex');ylabel('$\textbf{P}_{k|k-1},\textbf{P}_{k|k},\textbf{P}_{k|N}$','Interpreter','latex');
grid on
ax = gca;
